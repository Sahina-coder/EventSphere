...
const clampVal = (value: number, min: number, max: number) => Math.min(Math.max(value, min), max);
...
const GlowCursor = ({
  ...
  propsRef.current = {
    ...
    noiseStrength, idleFade, idleTimeout, fadeDuration, maxDevicePixelRatio, blendMode, enabled,
  };
  ...
  useEffect(() => {
    ...
    const updatePointer = (event: PointerEvent | MouseEvent) => {
      const x = clampVal(event.clientX, 0, window.innerWidth);
      const y = clampVal(window.innerHeight - event.clientY, 0, window.innerHeight);
    ...
    const render = (now: number) => {
      ...
      if (initialized) {
        const headEase = 1 - Math.pow(1 - clampVal(config.followSpeed, 0.01, 0.99), delta);
        const chainBase = clampVal(0.28 + config.followSpeed * 0.35, 0.08, 0.92);
      ...
      program.uniforms.uPointCount.value = clampVal(Math.round(config.trailLength), 2, MAX_POINTS);
      ...
      program.uniforms.uTaper.value = clampVal(config.trailTaper, 0, 1);
      ...
      program.uniforms.uHotspot.value = clampVal(config.hotspot, 0, 1);
      program.uniforms.uBrightness.value = Math.max(config.brightness, 0);
      program.uniforms.uOpacity.value = clampVal(config.opacity, 0, 1);
      program.uniforms.uPulseSpeed.value = config.pulseSpeed;
      program.uniforms.uNoiseStrength.value = clampVal(config.noiseStrength, 0, 1);
  ...
  return (
    <div ref={containerRef} className="glow-cursor-overlay">
