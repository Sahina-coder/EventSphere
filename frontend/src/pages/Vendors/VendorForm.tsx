...
const VendorForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    mutation.mutate({ name, service_type: serviceType, phone, email, availability: "Available" });
  };
  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-6 h-fit">
      <h2 className="font-display text-base font-semibold text-[var(--text)] mb-1">Add Vendor</h2>
      <p className="text-sm text-[var(--text-muted)] mb-5">Register a service provider.</p>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5">Vendor name</label>
          <input type="text" placeholder="ABC Catering" value={name} onChange={(e) => setName(e.target.value)} className={inputClass} required />
        </div>

        <div>
          <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5">Service type</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="tel" placeholder="9876543210" value={phone} onChange={(e) => setPhone(e.target.value)} className={inputClass} required />
          </div>
          <div>
            <label className="block text-xs font-medium text-[var(--text-muted)] mb-1.5">Email</label>
            <input type="email" placeholder="vendor@email.com" value={email} onChange={(e) => setEmail(e.target.value)} className={inputClass} required />
          </div>
        </div>
